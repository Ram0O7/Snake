...
let game = createGame(), mode = 'normal', lastTime = 0;
const music = createMusic((enabled, message) => {
 $('sound').textContent = enabled ? '♪ Mute sound' : '♪ Play sound';
 $('sound').setAttribute('aria-pressed', String(enabled));
 if (message) announce(message);
});
function syncMusic(){music.update(game.status, document.hidden);}
let records = {};
...
document.addEventListener('visibilitychange',()=>{if(document.hidden&&game.status==='running')pause();syncMusic();});window.addEventListener('blur',()=>{if(game.status==='running')pause();});
